  ...
  group('BudgetPeriod.currentRange', () {
    group('weekly', () {
      test('Wednesday → Monday..Sunday of that week', () {
        ...
        final (from, to) = BudgetPeriod.weekly.currentRange(
    ...
    group('biweekly', () {
      test('returns a 14-day window anchored to a Monday', () {
        final (from, to) = BudgetPeriod.biweekly.currentRange(
          now: DateTime(2026, 4, 29),
        );
      ...
      test(
        'the previous biweekly block is consistent with the current one',
        () {
          ...
          final (a, _) = BudgetPeriod.biweekly.currentRange(
            now: DateTime(2026, 4, 29),
          );
          final (b, _) = BudgetPeriod.biweekly.currentRange(
    ...
    group('monthly', () {
      test('mid-month → 1st through last day of that month', () {
        final (from, to) = BudgetPeriod.monthly.currentRange(
    ...
    group('semiannual', () {
      test('April → H1 (Jan 1 – Jun 30)', () {
        final (from, to) = BudgetPeriod.semiannual.currentRange(
          now: DateTime(2026, 4, 29),
        );
      ...
      test('June 30 is the last day of H1', () {
        final (_, to) = BudgetPeriod.semiannual.currentRange(
    ...
    group('annual', () {
      test('returns Jan 1 .. Dec 31 of the current year', () {
        final (from, to) = BudgetPeriod.annual.currentRange(
          now: DateTime(2026, 4, 29),
        );
        expect(from, DateTime(2026, 1, 1));
        expect(to, DateTime(2026, 12, 31));
