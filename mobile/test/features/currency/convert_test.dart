...
void main() {
  ...
  group('multiCurrencyNetWorth', () {
    test(
      'USD-only household with display=USD degenerates to plain sum',
      () {
    ...
    test(
      'missing-rate currency stays OUT of the total but surfaces in '
      ...
      () {
        ...
        expect(
          result.totalCents,
          100000,
          reason: 'JPY must be excluded from total when no rate is '
    ...
    test(
      'multiple non-display currencies each consult their own rate',
      () {
    ...
    test(
      'liabilities (negative balances) convert with the same rate',
      () {
    ...
    test(
      'empty account list returns zero totals across the board',
      () {
        final result = multiCurrencyNetWorth(
          accounts: const [],
          displayCurrency: 'USD',
