...
void main() {
  ...
  group('RecurringTransactionsRepository (integration)', () {
    ...
    test('create persists every field and returns the canonical row', () async {
      ...
      expect(
        ...
        isTrue,
        reason:
            'is_active must default to true at the DB level — '
    ...
    test('fetchAll orders by next_occurrence_date ascending', () async {
      ...
      final ours = all
          .where((r) => ['earliest', 'mid', 'latest'].contains(r.description))
          .toList();
      expect(
        ours.map((r) => r.description),
        ['earliest', 'mid', 'latest'],
        reason:
            'soonest-due rule must come first — that\'s what '
            'the future UI wants to surface at the top.',
      );
    }, skip: reason);

    test('update touches only the fields explicitly passed', () async {
      ...
      final updated = await repo.update(id: initial.id, description: 'after');
      ...
      expect(
        ...
        'OLD MERCHANT',
        reason:
            'unspecified fields must NOT be overwritten — the '
    ...
    test('skippedUntilDate set-vs-clear asymmetry: passing null in '
        'update does NOT clear; clearSkippedUntil does', () async {
    ...
    group('runScheduler', () {
      // Pick a fixed "today" in the past — migration 049 clamps
      // p_today to LEAST(p_today, CURRENT_DATE), so a future date
      // would silently become CURRENT_DATE and the tests' relative
      // date math (rule.next + N cycles == expected) would no
      // longer hold. 2024-06-15 is safely behind any plausible
      // CURRENT_DATE we'd run against; the setUp below wipes
      // sibling rules so isolation doesn't need an out-of-band
      // date.
      ...
      test('emits one transaction for a rule due today; advances the date '
          'and stamps last_emitted_at', () async {
        ...
        expect(txs.single['transaction_date'], '2024-06-15');
      ...
      test('multi-cycle catchup: a rule with backlog emits one row per '
          'missed cycle in one pass', () async {
        ...
        final rule = await repo.create(
          ...
          cadence: RecurrenceCadence.monthly,
          nextOccurrenceDate: DateTime.utc(2024, 3, 1),
          createdBy: harness.userId,
        );
        ...
        expect(
          ...
          reason:
              'four monthly cycles must emit in one pass — '
      ...
      test('skipped_until_date pauses emission but advances the date — '
          'rule never gets stuck in the past', () async {
        ...
        final rule = await repo.create(
          ...
          cadence: RecurrenceCadence.monthly,
          nextOccurrenceDate: DateTime.utc(2024, 5, 1),
          createdBy: harness.userId,
        );
        await repo.update(
          id: rule.id,
          skippedUntilDate: DateTime.utc(2024, 5, 31),
        );

        await repo.runScheduler(householdId: harness.householdId, today: today);
        ...
        expect(
          ...
          '2024-07-01',
          reason:
              'a long pause must still advance the date — without '
              'this the rule would loop forever with next_date stuck '
              'before skipped_until_date.',
        );
      }, skip: reason);

      test('inactive rules are ignored even when due', () async {
        final rule = await repo.create(
          householdId: harness.householdId,
          accountId: harness.accountId,
          amountCents: -100,
        ...
        expect(
          ...
          0,
          reason:
              'is_active=false must short-circuit emission so a '
      ...
      test(
        ...
        () async {
          // Today (test fixture) is June 15, 2024; rule next-date is July 1.
          final rule = await repo.create(
            householdId: harness.householdId,
            accountId: harness.accountId,
            amountCents: -100,
            ...
            cadence: RecurrenceCadence.monthly,
            nextOccurrenceDate: DateTime.utc(2024, 7, 1),
            createdBy: harness.userId,
          );
          ...
          expect(
            ...
            '2024-07-01',
            reason:
                'a future-dated rule must NOT advance — otherwise '
      ...
      // ── M3 (migration 049): clamp p_today to CURRENT_DATE ────────────
      test('p_today is clamped to CURRENT_DATE — a far-future p_today emits '
          'at most as if today were CURRENT_DATE', () async {
        // Pre-fix, a member could pass p_today := '2099-12-31'
        // and emit ~73 years of monthly catchup (~876 rows) for
        // a rule due today. With the clamp, the emission window
        // ends at CURRENT_DATE no matter what p_today is.
        //
        // Set the rule's next date to ~10 days before CURRENT_DATE
        // so under the clamp at most 1 monthly cycle is due. If
        // the clamp regresses, the count would jump to thousands
        ...
        await repo.create(
          ...
          accountId: harness.accountId,
          amountCents: -100,
          description: 'sched-clamp-attack',
          cadence: RecurrenceCadence.monthly,
          nextOccurrenceDate: ruleStart,
          createdBy: harness.userId,
        );

        await repo.runScheduler(
          householdId: harness.householdId,
          today: DateTime.utc(2099, 12, 31), // adversarial input
        );

        // At most 1 monthly cycle from `today - 10d` through
        // CURRENT_DATE. (Could be exactly 1 in the common case;
        // could be 2 if `today - 10d` and CURRENT_DATE straddle
        // a month boundary that triggers a second emission. Both
        // are far below the "decades of catchup" scale the
        // regression would produce.)
        final count = await countMatchingTx('sched-clamp-attack');
        expect(
          count,
          lessThanOrEqualTo(2),
          reason:
              'the LEAST(p_today, CURRENT_DATE) clamp must cap emissions '
              'at today; a future p_today must NOT drive the scheduler '
              'into the future. Without the clamp this would be ~876 '
              'rows for a 2099 p_today.',
        );
        expect(
          count,
          greaterThanOrEqualTo(1),
          reason:
              'the rule WAS due (next < CURRENT_DATE), so at least '
              'one emission must fire — the clamp shouldn\'t suppress '
              'legitimate catchup.',
        );
      }, skip: reason);
    });
