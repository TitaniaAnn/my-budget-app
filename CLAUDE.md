...
**Multi-currency — slice 1 only.** Migration 036 adds `households.display_currency CHAR(3) DEFAULT 'USD'` and an `fx_rates` table (composite PK on `(household_id, from_currency, to_currency, as_of_date)`, rate as `NUMERIC(18,8)`, CHECK rejects `rate <= 0` and `from == to`). [`FxRatesRepository.latestRate`](mobile/lib/features/currency/repositories/fx_rates_repository.dart) returns the most recent rate at or before a cutoff (or null — slice 1 does NOT silently apply 1/rate inversions; rates are explicit per direction). [`multiCurrencyNetWorth`](mobile/lib/features/currency/services/convert.dart) is the pure aggregation function: it sums accounts in the display currency directly, converts foreign-currency totals via the rate map, and surfaces a `missingRateCurrencies` set so the dashboard can warn instead of silently under-counting. Slice 1 wires only the dashboard's net worth card. Explicitly **not yet** converted: [`get_category_spending`](supabase/migrations/029_category_spending_dedupe_line_items.sql), budget caps vs spending in different currencies, the monthly report, and growth-advisor rules — they all still assume single-currency totals. A USD-only household with `display_currency='USD'` sees no behavioural change.
